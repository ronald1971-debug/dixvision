...
def _connect() -> sqlite3.Connection:
    ...
    con = sqlite3.connect(str(p), timeout=5.0, isolation_level=None, check_same_thread=False)
...
def issue_pairing(label: str, ttl_sec: int = _DEFAULT_TTL_SEC) -> Pairing:
    ...
    with _LOCK, _connect() as con:
        ...
        now = _utcnow_iso()
        expires_dt = datetime.now(UTC).timestamp() + max(60, int(ttl_sec))
        exp = datetime.fromtimestamp(expires_dt, tz=UTC).replace(microsecond=0).isoformat()
        con.execute(
            "INSERT INTO pairings(token,label,created_utc,expires_utc) VALUES(?,?,?,?)",
            (tok, label.strip()[:80] or "unnamed", now, exp),
        )
    get_writer().append_event(
        stream="SECURITY", kind="PAIRING_ISSUED", payload={"label": label, "expires_utc": exp}
    )
    return Pairing(
        token=tok,
        label=label,
...
def revoke_pairing(token: str) -> bool:
    ...
    if changed:
        get_writer().append_event(
            stream="SECURITY", kind="PAIRING_REVOKED", payload={"token_prefix": token[:6]}
        )
    return changed


def claim_pairing(token: str, *, bearer_token: str, device_fingerprint: str) -> str | None:
    ...
    get_writer().append_event(
        ...
        payload={"token_prefix": token[:6], "device": device_fingerprint[:20]},
    )
    return bearer_token


__all__ = ["Pairing", "issue_pairing", "list_pairings", "revoke_pairing", "claim_pairing"]
