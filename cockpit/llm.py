...
Every voice (INDIRA / DYON / GOVERNANCE / COGNITIVE_GOVERNANCE) gets
answers through this router. Each request declares a set of **capability
tags** (reason / code / translate / sentiment / long_context / realtime_web
...
Roles assigned at boot (see docs/AI_ROSTER.md):
    cognition_devin  \u2014 Dyon coder backend (external tool, not a system voice)
    anthropic_claude \u2014 GOVERNANCE reasoning + patch reviewer
    openai_gpt4o     \u2014 INDIRA strategy advisor + reasoning fallback
...
class LLMRouter:
    ...
    def _dispatch(self, p: Provider, prompt: str, system: str, max_tokens: int) -> LLMResponse:
        ...
        try:  # pragma: no cover
            ...
            if p.name in ("openai_gpt4o", "xai_grok", "deepseek", "perplexity", "cognition_devin"):
            ...
            return LLMResponse(text="", provider=p.name, model=p.model, error=repr(e))
        return LLMResponse(text="", provider=p.name, model=p.model, error="unsupported_provider")

    def _templated(
        self, prompt: str, system: str, required: frozenset[Capability], reason: str
    ) -> LLMResponse:
...
__all__ = ["Capability", "Provider", "ProviderStatus", "LLMResponse", "LLMRouter", "get_router"]
