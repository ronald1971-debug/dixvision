...
> **Note:** The default Docker `ENTRYPOINT` runs the **cockpit only** — it
> does NOT start the full trading system (intelligence, execution, governance
> engines).  For the full system in Docker, override the entrypoint:
>
> ```bash
> docker run --rm -p 8080:8080 -v dix-data:/data dix-vision:42.2 python start.py
> ```
>
> For local full-system usage run `python start.py` directly (no Docker).
