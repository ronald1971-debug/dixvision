...
class EpisodicMemory:
    ...
    def recent(self, *, strategy: str = "", symbol: str = "", limit: int = 200) -> list[Episode]:
        where, args = [], []
        if strategy:
        ...
        if symbol:
            ...
            args.append(symbol)
        sql = (
            "SELECT id, ts_utc, strategy, symbol, side, "
    ...
    def reward_window(self, strategy: str, *, n: int = 100) -> list[float]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT reward FROM episodes WHERE strategy=? ORDER BY id DESC LIMIT ?",
                (strategy, int(n)),
            ).fetchall()
    ...
    def _trim_locked(self) -> None:
        ...
        self._conn.execute(
            "DELETE FROM episodes WHERE id IN (SELECT id FROM episodes ORDER BY id ASC LIMIT ?)",
            (drop,),
        )
        self._conn.commit()
