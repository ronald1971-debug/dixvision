...
def submit(*, name: str, source: str, author: str, language: str = "python") -> CustomStrategy:
    ...
    with _lock:
        ...
        c.execute(
            ...
            (
                strategy_id,
                name.strip(),
                author.strip() or "operator",
                language,
                source,
                sha,
                StrategyState.DRAFT.value,
                "",
                now,
                now,
            ),
...
def list_strategies(
    ...
    with _lock:
        ...
        if state is None:
            ...
            rows = c.execute(
                "SELECT * FROM custom_strategies WHERE state=? ORDER BY updated_utc DESC",
                (state.value,),
            ).fetchall()
    return [_row_to_strategy(r) for r in rows]


def _transition(strategy_id: str, new_state: StrategyState, *, detail: str = "") -> CustomStrategy:
    now = _iso_now()
    with _lock:
        c = _connect()
        r = c.execute(
        ...
        c.execute(
            "UPDATE custom_strategies SET state=?, detail=?, updated_utc=? WHERE strategy_id=?",
...
def run_sandbox(strategy_id: str) -> CustomStrategy:
    ...
    if s.language != "python":
        return _transition(strategy_id, StrategyState.REJECTED, detail="language_not_python")
    ...
    try:
        ...
        try:
            ...
            pass
    return _transition(strategy_id, StrategyState.SANDBOX_OK, detail="sandbox_passed")
...
def promote_shadow(strategy_id: str) -> CustomStrategy:
    ...
    return _transition(strategy_id, StrategyState.SHADOW, detail="shadow_replay_enabled")
...
def promote_canary(strategy_id: str) -> CustomStrategy:
    ...
    return _transition(strategy_id, StrategyState.CANARY, detail="canary_active")


def request_go_live(
    strategy_id: str, *, operator_id: str = "operator", ttl_sec: int = 24 * 3600
) -> dict:
    ...
    req = request_approval(
        ...
        subject=strategy_id,
        payload={"name": s.name, "author": s.author, "sha256": s.source_sha256},
        ttl_sec=ttl_sec,
...
def promote_live(strategy_id: str) -> CustomStrategy:
    ...
    return _transition(strategy_id, StrategyState.LIVE, detail="operator_approved_live")


def retire(strategy_id: str, *, reason: str = "") -> CustomStrategy:
    return _transition(strategy_id, StrategyState.RETIRED, detail=f"retired:{reason}")
