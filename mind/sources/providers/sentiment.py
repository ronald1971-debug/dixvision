    ...
    def poll(self) -> list[SentimentPoint]:
        ...
        try:
            r = http_get(
                ...
                headers={"User-Agent": "dix-vision/42.2"},
                timeout_s=4.0,
            )
            if r.status != 200:
                self._mark_err(f"http={r.status}")
            ...
            self._mark_ok(1)
            return [
                SentimentPoint(source=self.name, polarity=pol, confidence=0.3, window_seconds=3600)
            ]
        except Exception as e:
            self._mark_err(repr(e))
            return []
...
class FearGreedProvider(_SentimentProviderBase):
    ...
    def poll(self) -> list[SentimentPoint]:
        ...
        try:
            ...
            self._mark_ok(1)
            return [
                SentimentPoint(source=self.name, polarity=pol, confidence=0.4, window_seconds=86400)
            ]
        except Exception as e:
            self._mark_err(repr(e))
            return []
