...
_SPECS: tuple[MarketSpec, ...] = (
    ...
    MarketSpec(
        "oneinch_api", "https://api.1inch.dev/", key_env="ONEINCH_API_KEY", key_required=True
    ),
    ...
    MarketSpec(
        "tiingo", "https://api.tiingo.com/", key_env="TIINGO_API_KEY", key_required=True, rps=1.0
    ),
    MarketSpec(
        "alpha_vantage",
    ...
    MarketSpec(
        "yahoo_finance_chart", "https://query1.finance.yahoo.com/v8/finance/chart/", rps=1.0
    ),
    # Institutional / regulatory
    MarketSpec("sec_edgar_fulltext", "https://efts.sec.gov/LATEST/search-index", rps=0.5),
    MarketSpec(
        "sec_edgar_form4",
        "https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&type=4",
        rps=0.5,
    ),
    MarketSpec("cftc_cot", "https://www.cftc.gov/dea/newcot/deacot.txt", rps=0.1),
)
