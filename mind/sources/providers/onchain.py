...
class SolanaRPCProvider(_OnchainProviderBase):
    ...
    def poll(self) -> list[OnchainEvent]:
        ...
        return [
            OnchainEvent(
                chain="solana", kind="endpoint_ping", payload={"endpoint": self._endpoint()}
            )
        ]
...
class GlassnodeProvider(_OnchainProviderBase):
    ...
    def poll(self) -> list[OnchainEvent]:
        ...
        try:
            ...
            self._mark_ok(1)
            return [
                OnchainEvent(chain="bitcoin", kind="metric", payload={"active_addresses": latest})
            ]
        except Exception as e:
            self._mark_err(repr(e))
            return []
...
class EthplorerProvider(_OnchainProviderBase):
    ...
    def poll(self) -> list[OnchainEvent]:
        ...
        try:
            r = http_get(
                "https://api.ethplorer.io/getTop?apiKey=freekey&criteria=trade&limit=10",
                timeout_s=4.0,
            )
            if r.status != 200:
                self._mark_err(f"http={r.status}")
            ...
            return [OnchainEvent(chain="ethereum", kind="top_traded", payload={"tokens": tokens})]
        except Exception as e:
            self._mark_err(repr(e))
            return []
