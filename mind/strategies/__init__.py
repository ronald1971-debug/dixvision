...
class TrendStrategy:
    ...
    def propose(self, *, symbol: str, features: dict[str, float]) -> StrategySignal | None:
        ...
        if abs(diff) < 0.002:
            return StrategySignal(self.name, symbol, "flat", 0.0, rationale="trend too weak")
...
class MeanReversionStrategy:
    ...
    def propose(self, *, symbol: str, features: dict[str, float]) -> StrategySignal | None:
        ...
        if abs(z) < 1.0:
            return StrategySignal(self.name, symbol, "flat", 0.0, rationale="z-score too small")
        side = "sell" if z > 0 else "buy"
        return StrategySignal(
            self.name, symbol, side, max(-1.0, min(1.0, -z / 3.0)), rationale=f"z-score={z:.2f}"
        )
...
class BreakoutStrategy:
    ...
    def propose(self, *, symbol: str, features: dict[str, float]) -> StrategySignal | None:
        ...
        if last >= high20 * 0.999:
            return StrategySignal(self.name, symbol, "buy", 0.6, rationale="20d high breakout")
        if last <= low20 * 1.001:
            return StrategySignal(self.name, symbol, "sell", 0.6, rationale="20d low breakdown")
        return StrategySignal(self.name, symbol, "flat", 0.0, rationale="inside range")
