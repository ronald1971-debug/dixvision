...
def _wall_ns() -> int:
    ...
    Uses ``_now_ns()`` directly. T0-4 exposes this as
...
def _estimate_bytes(value: Any) -> int:
    ...
    if isinstance(value, dict):
        return sum(_estimate_bytes(k) + _estimate_bytes(v) for k, v in value.items()) + 32
...
class KnowledgeStore:
    ...
    def compact(
        ...
        *,
        ...
        removed = 0
        with self._lock:
            victims = [k for k, e in self._data.items() if e.confidence < min_confidence]
            for k in victims:
    ...
    def restore(self, rows: list[dict[str, Any]]) -> None:
        ...
        with self._lock:
            ...
            self._bytes = 0
            for row in rows:
                size = int(row.get("size_bytes", _estimate_bytes(row["value"])))
                if size > self._max_bytes:
