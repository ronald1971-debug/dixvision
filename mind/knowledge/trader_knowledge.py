...
class TraderKnowledge:
        ...
        with self._lock:
            ...
            if row:
                if fields:
                    ...
                    self._conn.execute(
                        f"UPDATE traders SET {cols} WHERE id=?",
    ...
    def find_traders(
        self, q: str = "", style: str = "", region: str = "", limit: int = 50
    ) -> list[Trader]:
        where, args = [], []
        if q:
    ...
    # ---- strategies --------------------------------------------------
    def upsert_strategy(
        self, name: str, family: str = "", description: str = "", source_url: str = ""
    ) -> int:
        with self._lock:
    ...
    def add_statement(
        ...
        *,
        ...
        with self._lock:
            cur = self._conn.execute(
                "INSERT INTO statements (trader_id,strategy_id,kind,text,lang,en_summary,source_url,confidence,ledger_ref) "  # noqa: E501
                "VALUES (?,?,?,?,?,?,?,?,?)",
                (
                    trader_id,
                    strategy_id,
    ...
    def statements_for(self, trader_id: int, limit: int = 20) -> list[Statement]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT id,trader_id,strategy_id,kind,text,lang,en_summary,source_url,confidence,ledger_ref "  # noqa: E501
    ...
    def bump_mention(self, trader_id: int, now_utc: str) -> None:
        with self._lock:
            self._conn.execute(
                ...
                "ON CONFLICT(trader_id) DO UPDATE SET count=count+1, last_seen_utc=excluded.last_seen_utc",  # noqa: E501
                (trader_id, now_utc),
            )
            self._conn.commit()
