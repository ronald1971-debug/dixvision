...
class StrategyArbiter:
    def __init__(self, strategies: list[Strategy] | None = None) -> None:
        self._strategies: dict[str, Strategy] = {s.name: s for s in (strategies or list_builtins())}
        self._state: dict[str, StrategyState] = {n: StrategyState(name=n) for n in self._strategies}
