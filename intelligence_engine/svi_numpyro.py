    ...
    class _NumpyroSVIEngine:
        ...
        def infer(
            ...
            *,
            ...
            obs = numpy.array(arguments.observations, dtype=numpy.float32)

            def model(data):  # type: ignore[no-untyped-def]
                loc = numpyro.sample("loc", numpyro.distributions.Normal(0, 10))
                scale = numpyro.sample("scale", numpyro.distributions.HalfNormal(5))
                with numpyro.plate("data", len(data)):
                    numpyro.sample("obs", numpyro.distributions.Normal(loc, scale), obs=data)

            def guide(data):  # type: ignore[no-untyped-def]
                loc_q = numpyro.param("loc_q", 0.0)
                scale_q = numpyro.param(
                    "scale_q", 1.0, constraint=numpyro.distributions.constraints.positive
                )
                numpyro.sample("loc", numpyro.distributions.Normal(loc_q, 0.1))
                numpyro.sample("scale", numpyro.distributions.HalfNormal(scale_q))

            optimizer = numpyro.optim.Adam(0.01)
            svi = numpyro.infer.SVI(model, guide, optimizer, loss=numpyro.infer.Trace_ELBO())
            rng_key = jr.PRNGKey(arguments.random_seed)
            svi_state = svi.init(rng_key, obs)

            total_loss = 0.0
            for _ in range(200):
                svi_state, total_loss = svi.update(svi_state, obs)

            params = svi.get_params(svi_state)
            loc_val = float(params.get("loc_q", 0.0))
            scale_val = float(params.get("scale_q", 1.0))

            summaries: list[NumpyroSiteSummary] = []
            for i in range(spec.num_sites):
                summaries.append(
                    NumpyroSiteSummary(
                        name=f"site_{i}",
                        mean=loc_val if i == 0 else 0.0,
                        std=scale_val if i == 0 else 1.0,
                        effective_sample_size=float(len(obs)),
                        r_hat=1.0,
                        divergences=0,
                    )
                )
