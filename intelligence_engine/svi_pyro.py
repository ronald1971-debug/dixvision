    ...
    class _PyroSVIEngine:
        ...
        def infer(
            ...
            *,
            ...
            pyro.set_rng_seed(arguments.random_seed)
            pyro.clear_param_store()

            obs = torch.tensor(arguments.observations, dtype=torch.float32)

            def model(data):  # type: ignore[no-untyped-def]
                loc = pyro.param("loc", torch.zeros(spec.num_sites))
                scale = pyro.param(
                    "scale",
                    torch.ones(spec.num_sites),
                    constraint=torch.distributions.constraints.positive,
                )
                for i in range(spec.num_sites):
                    pyro.sample(f"site_{i}", pyro.distributions.Normal(loc[i], scale[i]))
                with pyro.plate("data", len(data)):
                    pyro.sample("obs", pyro.distributions.Normal(loc[0], scale[0]), obs=data)

            def guide(data):  # type: ignore[no-untyped-def]
                loc_q = pyro.param("loc_q", torch.zeros(spec.num_sites))
                scale_q = pyro.param(
                    "scale_q",
                    torch.ones(spec.num_sites),
                    constraint=torch.distributions.constraints.positive,
                ...
                for i in range(spec.num_sites):
                    pyro.sample(f"site_{i}", pyro.distributions.Normal(loc_q[i], scale_q[i]))
            ...
            total_loss = 0.0
            for _ in range(200):
                total_loss = svi.step(obs)

            summaries: list[SVISiteSummary] = []
            for i in range(spec.num_sites):
                loc_val = float(pyro.param("loc_q")[i].item())
                scale_val = float(pyro.param("scale_q")[i].item())
                summaries.append(
                    SVISiteSummary(
                        name=f"site_{i}",
                        mean=loc_val,
