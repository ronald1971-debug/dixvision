...
_BELIEF_VS_ENTRY_BLOCKLIST: Mapping[str, frozenset[EntryStyle]] = {
    ...
    "mean_reversion": frozenset({EntryStyle.BREAKOUT, EntryStyle.MOMENTUM}),
    ...
    "volatility_premium": frozenset({EntryStyle.MOMENTUM, EntryStyle.BREAKOUT}),
}
...
_RISK_ATTITUDE_VS_STOP_BLOCKLIST: Mapping[RiskAttitude, frozenset[StopStyle]] = {
...
_REGIME_VS_ENTRY_BLOCKLIST: Mapping[MarketRegime, frozenset[EntryStyle]] = {
    MarketRegime.TRENDING: frozenset({EntryStyle.MEAN_REVERSION}),
    MarketRegime.RANGING: frozenset({EntryStyle.BREAKOUT, EntryStyle.MOMENTUM}),
}
...
class IncompatibleCompositionError(ValueError):
    ...
    def __init__(self, findings: tuple[IncompatibilityFinding, ...]) -> None:
        if not findings:
            raise ValueError("IncompatibleCompositionError requires at least one finding")
...
def _check_philosophy(
    ...
    blocked_stops = _RISK_ATTITUDE_VS_STOP_BLOCKLIST.get(philosophy.risk_attitude, frozenset())
...
def _check_horizon(
    ...
    blocked_intervals = _HORIZON_VS_BAR_INTERVAL_BLOCKLIST.get(philosophy.time_horizon, frozenset())
...
def _check_regime(
    ...
    blocked = _REGIME_VS_ENTRY_BLOCKLIST.get(market_condition.regime, frozenset())
...
def compose(
    *,
    ...
    findings.extend(_check_unknown_discriminators(entry, exit_, risk, market_condition))
...
_RESERVED_CONVICTION_STYLES: frozenset[ConvictionStyle] = frozenset({ConvictionStyle.UNKNOWN})
