        ...
        for prov in _all_providers():
            ...
            for cand in results:
                ...
                if _is_frozen(cand.target_path):
                    tick.errors.append(f"refused_frozen_path:{cand.target_path}:{cand.url}")
                    continue
...
def _write_tick_events(tick: ScoutTick) -> None:
    ...
    for cand in tick.candidates:
        try:
            get_writer().write(
                "SYSTEM", "DISCOVERY_CANDIDATE", "system_monitor.weekly_scout", cand.as_dict()
            )
        except Exception:
            pass
...
def _should_fire_now() -> bool:
    ...
    return n.minute == mi and n.hour == ho and iso_dow == dow and (iso_week % weeks) == 0
...
def start(*, daemon: bool = True) -> None:
    ...
    with _lock:
        if _thread is not None and _thread.is_alive():
            return
        _stop.clear()
        _thread = threading.Thread(target=_loop, name="dix-weekly-scout", daemon=daemon)
        _thread.start()
...
def _epoch_now() -> float:
    n = utc_now()
    return n.timestamp() if n else _wall_ns() / 1_000_000_000
