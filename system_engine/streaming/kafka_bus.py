...
def kafka_producer_factory(config: KafkaConfig) -> Any:
    ...
    try:
        ...
        raise ImportError(
            "kafka_producer_factory requires the optional 'aiokafka' package "
            "— install with 'pip install aiokafka' "
            "(NEW_PIP_DEPENDENCIES in system_engine/streaming/kafka_bus.py "
            "flags this)."
        ) from exc
...
def kafka_consumer_factory(config: KafkaConfig, group: ConsumerGroup) -> Any:
    ...
    try:
        ...
        raise ImportError(
            "kafka_consumer_factory requires the optional 'aiokafka' package "
            "— install with 'pip install aiokafka' "
            "(NEW_PIP_DEPENDENCIES in system_engine/streaming/kafka_bus.py "
            "flags this)."
        ) from exc
