...
def nats_client_factory(config: NATSConfig) -> object:
    ...
    try:
        from nats.aio.client import Client as _NATSClient  # noqa: PLC0415
    except ImportError as exc:
        raise ImportError(
            "nats_client_factory requires the optional 'nats-py' package "
            "— install with 'pip install nats-py' "
            "(NEW_PIP_DEPENDENCIES in system_engine/streaming/nats_bus.py "
            "flags this)."
        ) from exc
    return _NATSClient()
...
def jetstream_factory(config: NATSConfig) -> object:
    ...
    try:
        ...
        raise ImportError(
            "jetstream_factory requires the optional 'nats-py' package "
            "— install with 'pip install nats-py' "
            "(NEW_PIP_DEPENDENCIES in system_engine/streaming/nats_bus.py "
            "flags this)."
        ) from exc
    return _NATSClient().jetstream()
