...
def pulsar_client_factory(config: PulsarConfig) -> Any:
    ...
    try:
        import pulsar  # noqa: PLC0415
    except ImportError as exc:
        raise ImportError(
            "pulsar_client_factory requires the optional 'pulsar-client' package "
            "— install with 'pip install pulsar-client' "
            "(NEW_PIP_DEPENDENCIES in system_engine/streaming/pulsar_bus.py "
            "flags this)."
        ) from exc
    return pulsar.Client(
        service_url=config.service_url,
        operation_timeout_seconds=config.operation_timeout_ns / 1_000_000_000,
        connection_timeout_ms=config.connection_timeout_ns // 1_000_000,
    )
