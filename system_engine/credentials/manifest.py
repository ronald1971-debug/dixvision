...
CREDENTIAL_BLUEPRINTS: Mapping[str, CredentialBlueprint] = MappingProxyType(
    {
        ...
        "cognition": CredentialBlueprint(
            # Devin AI — multi-step coding agent used by Dyon Chat for
            # connector authoring and patch tasks. Not a system voice;
            # an external tool accessed via the Cognition API.
            env_vars=("COGNITION_API_KEY",),
            signup_url="https://app.devin.ai/settings/integrations",
            free_tier=False,
            notes=("Devin AI coding agent. Token from Settings → Integrations → API keys."),
        ),
        # ----- News -----
