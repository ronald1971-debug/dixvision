...
def brian2_prototype_factory(
    *,
    ...
    try:
        ...
        raise ImportError(
            "brian2_prototype_factory requires 'brian2' — install with 'pip install brian2'"
        ) from exc
    ...
    eqs = """
    dv/dt = (v_leak - v + r_input * I) / tau_mem : 1
    I : 1
    v_leak : 1
    r_input : 1
    tau_mem : second
    ...
    G = NeuronGroup(
        1,
        eqs,
        threshold=f"v > {params.v_thresh}",
        reset=f"v = {params.v_reset}",
        refractory=params.refractory_steps * params.dt * second,
        dt=dt_sec * second,
        method="euler",
    )
    G.v = params.v_reset
    G.v_leak = params.v_leak
    G.r_input = params.r_input
    G.tau_mem = params.tau_mem * second

    spike_mon = SpikeMonitor(G)
    state_mon = StateMonitor(G, "v", record=True)

    net = Network(G, spike_mon, state_mon)

    spikes_list: list[bool] = []
    v_history: list[float] = []
    for step_idx in range(n_steps):
        G.I = current[step_idx]
        net.run(dt_sec * second)
        v_val = float(state_mon.v[0][-1]) if len(state_mon.v[0]) > 0 else params.v_reset
        v_history.append(v_val)
        spiked = len(spike_mon.t) > sum(spikes_list)
        spikes_list.append(spiked)

    spike_times = tuple(i for i, s in enumerate(spikes_list) if s)

    return LIFTrace(
        backend="brian2",
        spikes=tuple(spikes_list),
        v_history=tuple(v_history),
        spike_times_steps=spike_times,
        params=params,
    )
