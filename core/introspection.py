...
Every voice (INDIRA / DYON / GOVERNANCE / COGNITIVE_GOVERNANCE) can answer
operator questions about its role by reading its own Charter + the most
recent ledger events tagged to its accountability streams. No LLM required;
...
def introspect(
    ...
    recent_ledger_refs: Iterable[int] | None = None,
) -> Introspection:
    c = get_charter(voice)
    if c is None:
