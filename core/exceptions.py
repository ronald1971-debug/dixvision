...
class StructuredError(Exception):
    def __init__(
        self, message: str, code: str = "UNKNOWN", metadata: dict[str, Any] | None = None
    ) -> None:
...
class GovernanceViolation(StructuredError):
    def __init__(self, reason: str, action: str = "") -> None:
        super().__init__(
            f"Governance violation: {reason}", "GOV_VIOLATION", {"reason": reason, "action": action}
        )
...
class DomainViolation(StructuredError):
    ...
    def __init__(self, component: str, target_domain: str) -> None:
        super().__init__(
            f"{component} cannot access {target_domain} domain",
            "DOMAIN_VIOLATION",
