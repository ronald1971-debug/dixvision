...
def test_episode_budget_cap_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    # Patch MAX_EPISODE_STEPS to a small value so CI doesn't timeout
    # (the real value is 1_000_000 which is too slow for CI).
    _test_budget = 100
    monkeypatch.setattr(
        "evolution_engine.environments.anytrading_env.MAX_EPISODE_STEPS",
        _test_budget,
    )
    cfg = AnytradingConfig(
        prices=tuple(float(i + 1) for i in range(_test_budget + 10)),
        window_size=2,
    )
