...
def test_captures_every_n_events() -> None:
    ...
    eng = SnapshotEngine(
        {"m": CountingProjector()},
        SnapshotPolicy(events_per_snapshot=3, nanoseconds_per_snapshot=10**18, max_ring_size=16),
...
def test_captures_when_time_elapses() -> None:
    ...
    eng = SnapshotEngine(
        {"m": CountingProjector()},
        SnapshotPolicy(events_per_snapshot=10**9, nanoseconds_per_snapshot=100, max_ring_size=4),
        clock_monotonic_ns=fake_mono,
...
def test_ring_is_bounded() -> None:
    eng = SnapshotEngine(
        {"m": CountingProjector()},
        SnapshotPolicy(events_per_snapshot=1, nanoseconds_per_snapshot=10**18, max_ring_size=3),
    )
    for i in range(1, 11):
...
def test_latest_at_or_before_picks_correct_snapshot() -> None:
    eng = SnapshotEngine(
        {"m": CountingProjector()},
        SnapshotPolicy(events_per_snapshot=1, nanoseconds_per_snapshot=10**18, max_ring_size=16),
    )
    for i in [1, 2, 5, 9]:
        eng.on_event(_event(i))
...
def test_snapshot_payloads_are_deterministic() -> None:
    ...
    def make_engine() -> SnapshotEngine:
        ...
        return SnapshotEngine(
            {"m": CountingProjector()},
            SnapshotPolicy(
                events_per_snapshot=2, nanoseconds_per_snapshot=10**18, max_ring_size=16
            ),
...
def test_snapshot_is_immutable_after_capture() -> None:
    ...
    eng = SnapshotEngine(
        {"m": proj},
        SnapshotPolicy(events_per_snapshot=1, nanoseconds_per_snapshot=10**18, max_ring_size=4),
    )
