...
def run() -> int:
    ...
    def t_safe_mode():
        ...
        assert (
            not ok
        )  # trading blocked in safe mode (reason may be trading_not_allowed or safe_mode_active)
    ...
    def t_ledger_write():
        ...
        try:
            store = EventStore(db)
            ev = store.append("MARKET", "TEST_TRADE", "indira", {"asset": "BTCUSDT", "side": "BUY"})
    ...
    def t_severity_classifier():
        ...
        critical = HazardEvent(
            HazardType.DATA_CORRUPTION_SUSPECTED, HazardSeverity.CRITICAL, "test"
        )
    ...
    def t_snapshot():
        ...
        _orig = _os.environ.get("DIX_SNAPSHOTS_DIR")  # noqa: F841
