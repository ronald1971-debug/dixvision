...
def test_mind_routers_and_managers():
    ...
    order = om.submit(Order(client_id="c1", asset="BTCUSDT", side="BUY", size=0.1, price=50_000.0))
...
def test_state_writer_and_projectors():
    ...
    gp.apply(
        {"event_type": "GOVERNANCE", "sub_type": "DECISION", "payload": {"outcome": "APPROVED"}}
    )
...
def test_observability_stack():
    ...
    ae.register(
        AlertRule(
            name="feed_silence",
            predicate=lambda e: e.get("event_type") == "HAZARD",
            severity="HIGH",
        )
    )
...
def test_wallet_policy_check_and_consume_is_atomic():
    ...
    with wp._lock:
        c = wp._connect()
        row = c.execute("SELECT v FROM policy_meta WHERE k=?", (wp._BIRTH_KEY,)).fetchone()
    ...
    try:
        ...
        with wp._lock:
            ...
            if original_birth is not None:
                ...
                c.execute("DELETE FROM policy_meta WHERE k=?", (wp._BIRTH_KEY,))
            c.commit()
...
def test_autonomy_manager_user_controlled_blocks_auto():
    ...
    mgr = get_autonomy()
    mgr.transition(AutonomyMode.USER_CONTROLLED, operator_id="test", reason="unit-test")
...
def test_autonomy_manager_semi_auto_envelope():
    ...
    mgr.set_budget(
        ...
        AutonomyBudget(max_size_usd=50.0, max_trades_per_hour=3, auto_asset_allowed=False),
        operator_id="test",
    )
    mgr.transition(AutonomyMode.SEMI_AUTO, operator_id="test", reason="unit-test")
    ...
    mgr.transition(AutonomyMode.USER_CONTROLLED, operator_id="test", reason="restore")
...
def test_operator_approval_lifecycle():
    ...
    req2 = request_approval(ApprovalKind.DAILY_CAP_CHANGE, subject=subj2, ttl_sec=60)
...
def test_operator_single_person_approval():
    import uuid as _uuid
    ...
    req = request_approval(ApprovalKind.KILL_SWITCH_OVERRIDE, subject=subject, ttl_sec=60)
    # Single operator approval is sufficient (no two-person gate).
    r1 = approve(req.request_id, operator_id="ronald")
    assert r1.state is ApprovalState.GRANTED
...
def test_logger_concurrent_writes_do_not_interleave():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        threads = [threading.Thread(target=_spam, args=(lg, i)) for i, lg in enumerate(loggers)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
...
def test_wallet_policy_birth_single_clock_read():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        con.execute("CREATE TABLE IF NOT EXISTS policy_meta(k TEXT PRIMARY KEY, v TEXT)")
        calls = {"n": 0}
        _orig = _wp.utc_now
