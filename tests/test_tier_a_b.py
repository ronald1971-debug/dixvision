...
def test_charters_cover_core_voices():
    ...
    cs = all_charters()
    # Four canonical voices: INDIRA (execution cognition), DYON (system
    # perception), GOVERNANCE (financial control), COGNITIVE_GOVERNANCE
    # (epistemic integrity). No external-tool voices in the charter system.
    assert set(cs.keys()) == {
        Voice.INDIRA,
        Voice.DYON,
        Voice.GOVERNANCE,
        Voice.COGNITIVE_GOVERNANCE,
    }
    for v, c in cs.items():
...
def test_chat_routes_all_voices():
    ...
    chat = get_chat()
    for v in (Voice.INDIRA, Voice.DYON, Voice.GOVERNANCE, Voice.COGNITIVE_GOVERNANCE):
...
def test_execution_algos_shapes():
    ...
    assert len(plan_vwap(symbol="X", side="buy", qty=10.0, window_sec=600).children) == 12
    assert len(plan_twap(symbol="X", side="buy", qty=10.0, window_sec=600, slices=6).children) == 6
    assert len(plan_iceberg(symbol="X", side="buy", qty=10.0, show_size=2.0).children) == 5
    p = plan_pov(
        symbol="X",
        side="buy",
        qty=100.0,
        observed_volume_per_sec=10.0,
        participation=0.1,
        window_sec=120.0,
    )
...
def test_slippage_and_mev_guard():
    ...
    assert min_acceptable_price(mid=100.0, side="buy", exp_slip_bps=s.exp_slippage_bps) > 100.0
...
def test_risk_engine():
    ...
    size = position_sizing(
        equity=50_000, target_vol_annual=0.2, asset_vol_daily=0.02, regime=snap.regime
    )
    assert size >= 0


def test_wallet_policy_supervised():
    """WARMUP phase has been permanently removed.

    The policy now starts in SUPERVISED from day 0. Live signing is
    governed by daily USD caps (not a blanket lockout). The test
    verifies: snapshot returns SUPERVISED, live_signing_allowed is True,
    and warmup_days_remaining is 0 (backward-compat field always 0).
    """
    from security import wallet_policy as wp

    s = wp.snapshot()
    assert s.phase is wp.Phase.SUPERVISED
    assert s.live_signing_allowed is True
    assert s.warmup_days_remaining == 0  # backward-compat field, always 0
    # Daily cap gate still works: small notional within cap passes.
    ok, reason = wp.can_sign(
        "ethereum", "0xDEADBEEF000000000000000000000000000000AA", usd_notional=10.0
    )
    assert ok, f"expected can_sign ok, got {reason}"
...
def test_wallet_connect_watch_only():
    ...
    wc.connect_wallet(
        label="t",
        chain=wc.Chain.ETHEREUM,
        backend=wc.Backend.WATCH_ONLY,
        address="0xDEADBEEF000000000000000000000000000000BB",
    )
    assert not wc.can_sign(
        wc.Chain.ETHEREUM, "0xDEADBEEF000000000000000000000000000000BB", usd_notional=5.0
    )
