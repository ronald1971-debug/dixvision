...
def test_mean_reversion_belief_blocks_momentum_entry() -> None:
    ...
    assert any(
        f.reason is IncompatibilityReason.PHILOSOPHY_VS_ENTRY_STYLE for f in exc.value.findings
    )
...
def test_mean_reversion_belief_blocks_trailing_stop_exit() -> None:
    ...
    assert any(
        f.reason is IncompatibilityReason.PHILOSOPHY_VS_EXIT_STYLE for f in exc.value.findings
    )
...
def test_conservative_with_no_stop_blocked() -> None:
    ...
    assert any(
        f.reason is IncompatibilityReason.PHILOSOPHY_VS_RISK_ATTITUDE for f in exc.value.findings
    )
...
def test_scalp_horizon_blocked_on_daily_bars() -> None:
    ...
    assert any(f.reason is IncompatibilityReason.HORIZON_VS_TIMEFRAME for f in exc.value.findings)
...
def test_trending_regime_blocks_mean_reversion_entry() -> None:
    ...
    assert any(f.reason is IncompatibilityReason.REGIME_VS_ENTRY_STYLE for f in exc.value.findings)
...
def test_unknown_entry_style_rejected() -> None:
    ...
    assert any(f.reason is IncompatibilityReason.UNKNOWN_DISCRIMINATOR for f in exc.value.findings)
...
def test_unknown_sizing_style_rejected() -> None:
    with pytest.raises(IncompatibleCompositionError) as exc:
        compose(
            composition_id="c",
            decomposition_id="d",
            entry=_entry(),
            exit_=_exit(),
            risk=_risk(sizing=SizingStyle.UNKNOWN),
            timeframe=_timeframe(),
            market_condition=_market(),
        )
    assert any(f.reason is IncompatibilityReason.UNKNOWN_DISCRIMINATOR for f in exc.value.findings)
