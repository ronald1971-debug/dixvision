...
from system.time_source import now_ns as _now_ns
...
class SnapshotEngine:
    ...
    def _capture_if_due_locked(self) -> Snapshot | None:
        ...
        due = (
            ...
            or (self._clock_mono() - self._last_capture_mono_ns) >= policy.nanoseconds_per_snapshot
        )
        if not due:
            return None
        return self._capture_locked()
    ...
    def _capture_locked(self) -> Snapshot:
        projector_views = {name: proj.snapshot() for name, proj in self._projectors.items()}
        ...
        cursor_wall = self._last_wall_ns if self._last_wall_ns is not None else self._clock_wall()
        snap = Snapshot(
            cursor=SnapshotCursor(
