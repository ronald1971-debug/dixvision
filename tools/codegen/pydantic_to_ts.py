...
def _ts_literal(value: Any) -> str:
    """Render a Python JSON value as its TypeScript literal expression."""
    if value is None:
        return "null"
...
def _schema_to_ts(
    ...
    # Enum: pydantic emits `Literal[...]` and `StrEnum` as `{"enum": [...]}`.
    # Values may be str, int, float, bool, or None — render each as a TS literal.
    if "enum" in schema:
    ...
    if "allOf" in schema:
        parts = [_schema_to_ts(s, defs, interfaces, enums) for s in schema["allOf"]]
        joined = " & ".join(p for p in parts if p)
        return joined or "unknown"
    if not schema:
        return "unknown"
    raise TypeError(f"unsupported JSON-schema fragment: {schema!r}")
...
def _emit_named(
    ...
    if "enum" in schema:
        enums[name] = _Enum(name=name, values=list(schema["enum"]))
        return

    if schema.get("type") != "object":
        raise TypeError(f"unsupported named def {name!r}: {schema!r}")
...
def _render(
    ...
    for enum in enums.values():
        union = " | ".join(_ts_literal(v) for v in enum.values)
