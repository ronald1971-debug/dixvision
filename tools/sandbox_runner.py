...
_SKIP_TOP_DIRS = frozenset({"tests", "docs", "scripts", "mobile", "windows", "cloud"})
...
def _run(args: list[str], cwd: str, timeout: int, pypath: str | None = None) -> SandboxResult:
    try:
        ...
        return SandboxResult(
            ok=False, stage="spawn_failed", stdout="", stderr=str(exc), returncode=127
        )


def sandbox_import(
    module_path: Path, repo_root: Path, timeout: int = _DEFAULT_TIMEOUT
) -> SandboxResult:
    ...
    if not module_path.is_file():
        return SandboxResult(
            ...
            stage="missing",
            stdout="",
            stderr=f"no such file: {module_path}",
            returncode=2,
        )
    rel = module_path.relative_to(repo_root)
    if rel.parts and rel.parts[0] in _SKIP_TOP_DIRS:
        return SandboxResult(
            ok=True, stage="skipped", stdout=f"skipped {rel}", stderr="", returncode=0
        )
    ...
    with tempfile.TemporaryDirectory(prefix="dix-sandbox-") as tmp:
        ...
        return _run(
            [sys.executable, "-E", "-s", "-W", "error", "-c", script],
            cwd=str(dst_root),
            timeout=timeout,
            pypath=str(dst_root),
        )
...
def sandbox_authority_lint(repo_root: Path, timeout: int = _DEFAULT_TIMEOUT) -> SandboxResult:
    """Run tools/authority_lint.py; pipeline fails if any violation is found."""
    return _run(
        [sys.executable, "-E", "-s", "tools/authority_lint.py"],
        cwd=str(repo_root),
        timeout=timeout,
...
def sandbox_unit_tests(
    repo_root: Path, pattern: str | None = None, timeout: int = 180
) -> SandboxResult:
...
def sandbox_dep_scan(requirements: Path, timeout: int = _DEFAULT_TIMEOUT) -> SandboxResult:
    ...
    if not requirements.is_file():
        return SandboxResult(
            ok=True, stage="noop", stdout="no requirements.txt", stderr="", returncode=0
        )
    ...
    if issues:
        return SandboxResult(
            ok=False, stage="dep_scan", stdout="", stderr="\n".join(issues), returncode=1
        )
    return SandboxResult(
        ok=True, stage="dep_scan", stdout=f"scanned {requirements}", stderr="", returncode=0
    )
...
def main() -> None:
    ...
    parser.add_argument("target", type=Path, help="path to .py module to sandbox-import")
    ...
    out = {
        ...
        "deps": sandbox_dep_scan(ns.repo / "requirements.txt", ns.timeout).to_dict(),
    }
