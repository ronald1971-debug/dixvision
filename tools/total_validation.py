...
def _phase_7_file_usage(
    ...
    for i, p in enumerate(files):
        ...
        is_entrypoint = (
            ...
            or rel == "start.py"
            or rel == "dix.py"
            or rel == "diagnose_foundation.py"
            or rel == "startup_test.py"
            or rel.startswith("windows/")
        )
        # Structural/tooling exclusions: empty __init__.py files are
        # package markers (not dead code), and docs/ tools are one-shot
        # audit scripts that are never imported at runtime.
        is_structural = (
            rel.endswith("/__init__.py")
            or rel.startswith("docs/")
        )
        all_callers = importers + ext_callers
        is_live = all_callers or feature_links or is_entrypoint or is_structural
        status = "USED" if is_live else "DEAD"
        if status == "DEAD":
            dead += 1
        rows.append(
            (
