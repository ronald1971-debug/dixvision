 ...
 *
 * Backend: GET /api/research/smart_money (trader KB projection).
 * Falls back to SEED + drift when backend unavailable.
 */
interface Wallet {
  addr: string;
  label: string;
...
export function SmartMoneyTracker() {
  ...
  useEffect(() => {
    const load = () =>
      fetch("/api/research/smart_money")
        .then((r) => r.json())
        .then((d) => {
          if (Array.isArray(d.wallets) && d.wallets.length > 0) {
            setWallets(
              d.wallets.map((w: Record<string, unknown>) => ({
                addr: String(w.addr ?? ""),
                label: String(w.label ?? ""),
                pnl_30d: Number(w.pnl_30d ?? 0),
                win_rate: Number(w.win_rate ?? 0),
                last_trade: {
                  ts: Number((w.last_trade as Record<string, unknown>)?.ts ?? Date.now()),
                  symbol: String((w.last_trade as Record<string, unknown>)?.symbol ?? "BTC"),
                  side: ((w.last_trade as Record<string, unknown>)?.side as "BUY" | "SELL") ?? "BUY",
                  size: Number((w.last_trade as Record<string, unknown>)?.size ?? 0),
                },
                follow: Boolean(w.follow),
              })),
            );
            setLive(true);
          }
