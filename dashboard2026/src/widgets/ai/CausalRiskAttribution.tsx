...
export function CausalRiskAttribution() {
  ...
  useEffect(() => {
    const load = () =>
      fetch("/api/research/causal_attribution")
        .then((r) => r.json())
        .then((d) => {
          if (Array.isArray(d.factors) && d.factors.length > 0) {
            setRows(d.factors as FactorRow[]);
            setLive(true);
          }
        })
        .catch(() => setLive(false));
    load();
    // Fall back to simulation drift when backend is unavailable.
  ...
  return (
    <section className="flex h-full flex-col rounded border border-border bg-surface">
      <header className="border-b border-border px-3 py-2 flex items-start justify-between">
        ...
        <span className={`text-[10px] px-1.5 py-0.5 rounded ${live ? 'bg-emerald-900 text-emerald-300' : 'bg-amber-900 text-amber-300'}`}>
          {live ? 'live' : 'mock'}
        </span>
      </header>
