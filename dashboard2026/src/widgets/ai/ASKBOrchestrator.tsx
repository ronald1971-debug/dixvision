 ...
 *
 * A single operator question is dispatched to multiple sub-agents in
 * parallel (data / news / research / analytics / risk). Each agent
 * surfaces a partial answer with its source, confidence, and elapsed
 * time. The orchestrator synthesizes the partials into a composite.
 *
 * Backend wiring: Dispatch POSTs to /api/research/submit; results are
 * polled from /api/research/tasks. The mock parallel simulation runs
 * immediately for UI responsiveness; the backend result arrives async
 * and is surfaced in the composite when ready.
 */
...
const INITIAL: Record<AgentKey, AgentState> = {
  ...
  research: { key: "research", label: "Research", status: "idle", elapsed_ms: 0 },
  analytics: { key: "analytics", label: "Analytics", status: "idle", elapsed_ms: 0 },
...
function mockPartial(q: string, key: AgentKey): AgentState {
  ...
  switch (key) {
    case "data":
      return {
        key, label: "Data", status: "done", elapsed_ms: 240,
        partial: hit("btc")
          ? "BTC last $67,420 · 24h vol $42B · 30d vol +18%"
          : "Top movers: NVDA +3.2%, TSLA -1.8%, AAPL +0.4%",
        source: "binance.spot · tradingview", confidence: 0.86,
      };
    case "news":
      return {
        key, label: "News", status: "done", elapsed_ms: 410,
        partial: hit("etf")
          ? "BlackRock IBIT inflows +$420M; weekly net +$1.1B."
          : "CoinDesk: macro print due Thursday; positioning skewed long.",
        source: "coindesk.rss · ap.wire", confidence: 0.74,
      };
    case "research":
      return {
        key, label: "Research", status: "done", elapsed_ms: 720,
        partial: hit("perps")
          ? "HL funding flipped negative on majors; basis -3bps annualized."
          : "Smart-money 7d net: BUY · top wallets long majors.",
        source: "askb.research-mock", confidence: 0.66,
      };
    case "analytics":
      return {
        key, label: "Analytics", status: "done", elapsed_ms: 320,
        partial: "BeliefState confidence 0.71 · PressureVector momentum +0.34",
        source: "intelligence_engine", confidence: 0.82,
      };
    case "risk":
      return {
        key, label: "Risk", status: "done", elapsed_ms: 180,
        partial: "VaR 1d 0.9% · DD floor 4.2% · CANARY size cap 1% notional",
        source: "padlock_floors", confidence: 0.91,
      };
  }
}
...
export function ASKBOrchestrator() {
  ...
  useEffect(() => {
    if (!backendResult) setComposite(summary);
  }, [summary, backendResult]);
  ...
  const ask = () => {
    ...
    // Launch mock simulation immediately for UI responsiveness.
    setAgents(Object.values(INITIAL).map((a) => ({ ...a, status: "running" as const })));
    const keys: AgentKey[] = ["data", "news", "research", "analytics", "risk"];
    keys.forEach((k, idx) => {
      const t = window.setTimeout(
        () => setAgents((prev) => prev.map((a) => (a.key === k ? mockPartial(q, k) : a))),
        180 + idx * 140,
      );
      timers.current.push(t);
    });

    // Also dispatch to the backend for a real cognitive answer.
    fetch("/api/research/submit", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ task_type: "ASKB_RESEARCH", query: q }),
    })
      .then((r) => r.json())
      .then((task) => {
        if (!task.id) return;
        setLive(true);
        // Poll tasks until this task completes.
        let attempts = 0;
          ...
          fetch("/api/research/tasks")
            ...
            .then((data) => {
              const found = (data.tasks ?? []).find(
                (t: Record<string, unknown>) => t.id === task.id,
              );
              if (found?.status === "completed" && found.result) {
                setBackendResult(String(found.result));
                setComposite(
                  `[cognitive] ${found.result}`,
                );
                if (pollRef.current !== null) window.clearInterval(pollRef.current);
                pollRef.current = null;
              }
      ...
      <div className="flex flex-1 flex-col gap-2 overflow-auto p-3 text-[12px]">
            ...
            >
              ...
              {a.status === "done" ? (
                <>
                  ...
                  <div className="text-[10px] text-slate-500">{a.elapsed_ms} ms</div>
                </>
              ) : (
        ...
        <div className="rounded border border-border bg-bg/40 p-2 text-[11px] text-slate-300">
          <h4 className="mb-1 font-mono text-[10px] uppercase tracking-wider text-slate-500">
            composite{backendResult ? " · cognitive" : ""}
          </h4>
          {composite ? (
            <p>{composite}</p>
          ) : (
