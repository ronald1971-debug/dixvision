 ...
 * sentiment score the projection layer uses for damping.
 *
 * Backend: GET /api/research/news/multilingual (NewsKnowledgeIndex
 * projection + ML translation gateway once provisioned).
...
const KNOWN_LANGS = new Set<string>(["EN", "JA", "ZH", "KO", "DE", "FR"]);
...
export function MultilingualNewsFusion() {
  ...
  useEffect(() => {
    const load = () =>
      fetch("/api/research/news/multilingual")
        .then((r) => r.json())
        .then((d) => {
          if (Array.isArray(d.items) && d.items.length > 0) {
            setItems(
              d.items.map((it: Record<string, unknown>) => ({
                ...
                lang: (KNOWN_LANGS.has(String(it.lang)) ? it.lang : "EN") as Lang,
                original: String(it.original ?? ""),
                translated: String(it.translated ?? ""),
                sentiment: Number(it.sentiment ?? 0),
              })),
            );
            setLive(true);
          }
        ...
        prev.map((it) => ({
          ...
          sentiment: Math.max(
            ...
            Math.min(
              1,
              it.sentiment +
                (Math.sin(Date.now() / 6_000 + it.ts) - 0.5) * 0.04,
            ),
          ),
        })),
      );
