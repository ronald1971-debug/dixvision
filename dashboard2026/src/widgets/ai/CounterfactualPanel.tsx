 ...
 *
 * Pick any historical trade from the audit ledger and re-simulate
 * the outcome under alternative parameters: tighter / looser SL,
 * scaled entry, opposite side, or "no trade". PnL is recomputed
 * from a deterministic surrogate model.
 *
 * Backend: GET /api/research/decisions — returns past trades from
 * the LedgerBridge. Falls back to SAMPLE_TRADES when unavailable.
 */
interface PastTrade {
  id: string;
  ts_iso: string;
...
export function CounterfactualPanel() {
  const [trades, setTrades] = useState<PastTrade[]>(SAMPLE_TRADES);
  const [live, setLive] = useState(false);
  const [selected, setSelected] = useState<string>(SAMPLE_TRADES[0].id);

  useEffect(() => {
    fetch("/api/research/decisions")
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d.trades) && d.trades.length > 0) {
          const parsed: PastTrade[] = d.trades.map(
            (t: Record<string, unknown>) => ({
              id: String(t.id ?? ""),
              ts_iso: String(t.ts_iso ?? ""),
          ...
          // Only mark live if the backend returned ledger entries, not samples.
          if (d.source === "ledger") setLive(true);
        }
      })
      .catch(() => setLive(false));
  }, []);
  ...
  const trade = useMemo(
    () => trades.find((t) => t.id === selected) ?? trades[0],
    [selected, trades],
  );
      ...
      <header className="border-b border-border px-3 py-2 flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">
            replay any past trade under alternative parameters · surrogate model
          </p>
        </div>
        <span
          ...
          >
            {trades.map((t) => (
              <option key={t.id} value={t.id}>
                {t.symbol} · {t.side} ·{" "}
                {t.ts_iso ? new Date(t.ts_iso).toLocaleString() : t.id}
              </option>
            ))}
          </select>
        </label>
