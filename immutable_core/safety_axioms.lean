...
axiom S6_frozen_hot_path :
  ∀ (m : Module),
    m ∈ {mind.fast_execute, system.fast_risk_cache, interrupt.*} →
      Changes(m) must pass SandboxPipeline ∧ OperatorApproval
    -- Hot-path code only changes through operator-approved sandbox pipeline.
...
-- routed through operator approval (manifest §22).
