...
class SystemIdentity:
    forbidden_behaviors: frozenset[str] = field(
        default_factory=lambda: frozenset(
            {
                "martingale",
                "unbounded_leverage",
                "runtime_core_mutation",
                "direct_dyon_trading_execution",
                "blocking_hazard_loop_in_hot_path",
                "silent_state_modification",
            }
        )
    )
    required_behaviors: frozenset[str] = field(
        default_factory=lambda: frozenset(
            {
                "deterministic_replay",
                "full_audit_trace",
                "governance_pre_approval_for_execution",
                "immutable_ledger_logging",
            }
        )
    )
