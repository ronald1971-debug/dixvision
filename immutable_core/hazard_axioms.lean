...
-- ── F. Override gate ────────────────────────────────────────────────
axiom H10_override_requires_operator_approval :
  ∀ (op : HazardOverride),
    op.effect = "clear_active_hazards" ∨ op.effect = "force_normal"
      → op.approvers.length ≥ 1
    -- Clearing active hazards or forcing NORMAL mode requires
    -- operator approval (single-operator authority).
