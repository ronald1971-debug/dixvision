...
def run_pipeline(path: Path, repo_root: Path) -> PipelineReport:
    ...
    writer.append_event(
        stream="GOVERNANCE", kind="PATCH_PROPOSED", payload={"patch_id": pid, "path": rel}
    )
    ...
    report.verdicts.append(
        PatchVerdict(
            patch_id=pid,
            path=rel,
            stage=Stage.AUTHORITY_LINT,
            ok=r_lint.ok,
            detail=r_lint.stage,
            stderr=r_lint.stderr,
        )
    )
    if not r_lint.ok:
        writer.append_event(
    ...
    report.verdicts.append(
        PatchVerdict(
            patch_id=pid,
            path=rel,
            stage=Stage.DEP_SCAN,
            ok=r_deps.ok,
            detail=r_deps.stage,
            stderr=r_deps.stderr,
        )
    )
    if not r_deps.ok:
        writer.append_event(
            stream="GOVERNANCE",
            kind="PATCH_REJECTED",
            payload={"patch_id": pid, "at": Stage.DEP_SCAN.value, "stderr": r_deps.stderr[-500:]},
        )
        return report

    writer.append_event(
        stream="GOVERNANCE", kind="PATCH_SANDBOX_PASS", payload={"patch_id": pid, "path": rel}
    )
    return report


def run_pipeline_with_tests(
    path: Path, repo_root: Path, pattern: str | None = None
) -> PipelineReport:
    ...
    report.verdicts.append(
        PatchVerdict(
            ...
            path=report.path,
            stage=Stage.UNIT_TESTS,
            ok=r_unit.ok,
            detail=r_unit.stage,
            stderr=r_unit.stderr,
        )
    )
    writer = get_writer()
    if not r_unit.ok:
        ...
        writer.append_event(
            stream="GOVERNANCE",
            kind="PATCH_UNIT_PASS",
            payload={"patch_id": report.patch_id, "path": report.path},
        )
    return report
...
def main() -> None:
    ...
    parser.add_argument(
        "--check-pr", action="store_true", help="scan every .py file changed in this PR"
    )
    ...
    if ns.check_pr:
        ...
        for f in changed:
            ...
            print(
                f"[{'PASS' if rep.ok else 'FAIL'}] {rep.path} "
...
__all__ = ["Stage", "PatchVerdict", "PipelineReport", "run_pipeline", "run_pipeline_with_tests"]
