...
class Authenticator:
    ...
    def issue(self, principal: str) -> Session:
        ...
        sess = Session(principal=principal, issued_at=issued, expires_at=expires, token=token)
        with self._lock:
    ...
    def verify(self, token: str) -> Session | None:
        ...
        if sess is None:
            return None
        if _wall_ns() / 1_000_000_000 > sess.expires_at:
            with self._lock:
