...
def plan_twap(
    *, symbol: str, side: str, qty: float, window_sec: float, slices: int = 12
) -> AlgoPlan:
    slices = max(1, int(slices))
    step = window_sec / slices
    qtys = _equal_slices(qty, slices)
    children = tuple(
        ChildOrder(seq=i, offset_sec=i * step, qty=q, note=f"twap_slice {i + 1}/{slices}")
...
def plan_iceberg(
    *, symbol: str, side: str, qty: float, show_size: float, refill_sec: float = 3.0
) -> AlgoPlan:
    if show_size <= 0:
    ...
    children = tuple(
        ChildOrder(seq=i, offset_sec=i * refill_sec, qty=q, note=f"iceberg_slice {i + 1}/{n}")
        for i, q in enumerate(qtys)
    )
...
def plan_pov(
    *,
    ...
    children = tuple(
        ChildOrder(
            seq=i, offset_sec=i * step_sec, qty=q, note=f"pov_{participation:.0%} {i + 1}/{n}"
        )
        for i, q in enumerate(qtys)
    )
...
__all__ = ["ChildOrder", "AlgoPlan", "plan_vwap", "plan_twap", "plan_iceberg", "plan_pov"]
