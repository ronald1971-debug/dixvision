...
class RuntimeGuardian:
    ...
    def start(self) -> None:
        with self._lock:
            if self._started:
                return
            self._started = True
            self._thread = threading.Thread(target=self._loop, daemon=True, name="DIX-Guardian")
            self._thread.start()
    ...
    def _loop(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                try:
                    trigger_kill_switch(f"guardian_unexpected_failure:{e}", "runtime_guardian")
                except Exception:
                    pass
    ...
    def _check(self) -> None:
        ...
        if state.last_heartbeat_ns > 0:
            age_ns = _monotonic_ns() - state.last_heartbeat_ns
            if age_ns > self._heartbeat_timeout_ns:
                self._state_mgr.set_mode("HALTED")
                trigger_kill_switch(f"stale_heartbeat:{age_ns / 1e9:.1f}s", "runtime_guardian")
                return
        if state.health < 0.3:
            trigger_kill_switch(f"critical_health:{state.health:.2f}", "runtime_guardian")
